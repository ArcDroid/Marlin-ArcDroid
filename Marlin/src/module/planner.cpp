...
bool Planner::_populate_block(block_t * const block, bool split_move,
  ...
  // Enable active axes
  #if ANY(CORE_IS_XY, MARKFORGED_XY, IS_SCARA)
  ...
  if (!block->steps.a && !block->steps.b && !block->steps.c) {    // Is this a retract / recover move?
    ...
    accel = CEIL((esteps || TERN(HAS_CUTTER, block->cutter_power, false) ? settings.acceleration : settings.travel_acceleration) * steps_per_mm);
