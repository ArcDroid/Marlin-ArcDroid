...
void _internal_move_to_destination(const_feedRate_t fr_mm_s/*=0.0f*/
  ...
  if (TERN0(IS_KINEMATIC, is_fast))
    TERN(IS_KINEMATIC, prepare_fast_move_to_destination(), prepare_line_to_destination());
  else
...
void prepare_line_to_destination() {
  ...
  #if EITHER(MORGAN_SCARA, MP_SCARA) && FALSE
  ...
  void homeaxis(const AxisEnum axis) {
    ...
    //
      ...
      if ((TERN0(X_SENSORLESS, axis == X_AXIS) || TERN0(Y_SENSORLESS, axis == Y_AXIS)) && backoff[axis]) {
        ...
        do_homing_move(axis, backoff_length, homing_feedrate(axis));  #if IS_SCARA
...
void set_axis_is_at_home(const AxisEnum axis) {
  ...
  void update_workspace_offset(const AxisEnum axis) {
    ...
    if (DEBUGGING(LEVELING)) DEBUG_ECHOLNPAIR("Axis ", AS_CHAR(XYZ_CHAR(axis)), " home_offset = ", home_offset[axis], " position_shift = ", position_shift[axis], " rotation = ", offset_rotation * 180);
  }
#endif
