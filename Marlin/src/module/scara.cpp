...
float segments_per_second = TERN(AXEL_TPARA, TPARA_SEGMENTS_PER_SECOND, SCARA_SEGMENTS_PER_SECOND);
  ...
  void forward_kinematics(const_float_t a, const_float_t b) {
    const float a_sin = sin(RADIANS(a)) * scara_L1,
                ...
                b_sin = sin(RADIANS(SUM_TERN(MP_SCARA, b, a))) * scara_L2,
                b_cos = cos(RADIANS(SUM_TERN(MP_SCARA, b, a))) * scara_L2;
  ...
  void scara_set_axis_is_at_home(const AxisEnum axis) {
    if (axis == Z_AXIS)
      ...
      LOOP_XYZ(i) homeposition[i] = base_home_pos((AxisEnum)i);
        ...
        forward_kinematics(SCARA_A_HOME + scara_home_offset.a, SCARA_B_HOME + scara_home_offset.b);
